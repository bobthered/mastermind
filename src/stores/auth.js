...
const store = () => {
  ...
  if ( typeof localStorage !== 'undefined') value = JSON.parse(localStorage.getItem('token') || JSON.stringify(''))
  ...
  const verify = async() => {
    ...
    await new Promise(res=>{
      // emit to server
      socket.emit('verifyToken', {token: JSON.parse(localStorage.getItem('token'))}, ({error, token})=> {
